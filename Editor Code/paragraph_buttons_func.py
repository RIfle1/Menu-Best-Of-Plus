...
def p_new_save():
    ...
    if f'{p_new_s_id}' == f'{p_new_c_p_id}':
        if p_new_text_length != 1:
            c.execute("INSERT INTO paragraphs_list VALUES (:s_id, :pl_id, :p_text, :npc_id, :mst_id)",
                      {
                          "s_id": f"{p_new_s_id}",
                          "pl_id": f"{p_new_s_id}_{id.conv('p_id', p_new_new_p_id)}",
                          "p_text": str(p_new_paragraph_text_entry.get("1.0", "end")),
                          "npc_id": "Nothing for now",
                          "mst_id": "Nothing for now"
                      })
