...
def CH4_component():  # Define the "Character 4" component and if it has to be displayed or not
    ...
    CH4 = tkinter.Button(characters_tab, text=player4_name, bd=0, bg="#FFFFFF", fg="#4285F4", width=63, height=2, font="Montserrat", relief=FLAT, command=CH4_action)
    CH4['font'] = myFont
    if Character4_set == 1:
...
def DoneButton_action():  # When "done" button is pressed
    ...
    # This following lines is for get which are write in the text box
    if active_character == 1:
        player1_name = ""
